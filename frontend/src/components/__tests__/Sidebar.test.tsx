...
describe("Sidebar", () => {
  ...
  it("renders the collapse control when a collapse handler is provided", () => {
    ...
    expect(screen.getByRole("navigation", { name: "BucketReef navigation" })).toHaveClass("pb-3");
    expect(screen.getByText("BucketReef").parentElement).toContainElement(collapseButton);
  ...
  it("renders footer content below navigation on desktop", () => {
    ...
    const nav = screen.getByRole("navigation", { name: "BucketReef navigation" });
