...
export default function PortalAddPeopleWorkflow({ accountId, spaceId, spaceName, existingRoles, disabled, onClose, onAdded }: {
  ...
  useEffect(() => {
    ...
    void listPortalStorageSpaceShareCandidates(accountId, spaceId).then(value => {
      ...
      if (active) setLoadError(extractApiError(cause, t({ en: "Unable to load people.", fr: "Impossible de charger les personnes.", de: "Personen können nicht geladen werden." })));
  ...
  return <SettingsWorkflowForm
    ...
    breadcrumbs={portalBreadcrumbs(
      ...
      { label: spaceName }, { label: t({ en: "Add people", fr: "Ajouter", de: "Hinzufügen" }) },
    )}
    ...
    onClose={onClose} submitLabel={t({ en: "Add people", fr: "Ajouter", de: "Hinzufügen" })}
    busyLabel={t({ en: "Adding...", fr: "Ajout...", de: "Wird hinzugefügt..." })}
    onSubmit={async () => {
      setError(null);
      try {
        ...
        setError(extractApiError(cause, t({ en: "Unable to add people.", fr: "Impossible d'ajouter ces personnes.", de: "Personen können nicht hinzugefügt werden." })));
      }
    }}>
    {requestMessage && <UiInlineMessage tone="success" role="status">{requestMessage} {" "}
      <Link to="/portal/requests" className="text-primary hover:underline">{t({ en: "Open Help requests", fr: "Ouvrir les demandes d'aide", de: "Hilfeanfragen öffnen" })}</Link>
    </UiInlineMessage>}
    <SettingsSection presentation="compact" title={t({ en: "People", fr: "Personnes", de: "Personen" })}
      description={t({
