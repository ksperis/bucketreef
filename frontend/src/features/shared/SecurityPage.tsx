  ...
  return <div className="settings-compact" data-profile-view="security">
    ...
    <SettingsSection presentation="compact" title={text("sessions")} description={text("sessionsHelp")}>
      <ResourceStatus resource={sessions} errorKey="loadSessionsError">
        {openSessions.length === 0 && <p className="py-3 text-sm text-[var(--ui-text-muted)]">{text("noSessions")}</p>}
    ...
    {panel && <ProfileDialog initialFocusRef={panel === "add" || panel === "password" ? firstInput : undefined} title={text(panelTitle)} onClose={guard.requestClose} closeOnBackdropClick={!busy} closeOnEscape={!busy} maxWidthClass={panel === "keys" || panel === "identities" ? "max-w-2xl" : "max-w-lg"}>
      <div className="settings-form space-y-4">
        {panel === "keys" && <><ul>{keys.data.map((key) => <li key={key.id} className="border-b border-[var(--ui-border-soft)] py-3 first:pt-0"><div className="flex flex-wrap items-center justify-between gap-3"><strong className="min-w-0 break-words text-sm">{key.name}</strong><ProfileButton variant="secondary" disabled={busy || Boolean(required && keys.data.length === 1)} onClick={() => confirm({ kind: "key", value: key })}>{text("removeKey")}</ProfileButton></div><p className="mt-1 text-[13px] text-[var(--ui-text-muted)]">{text("created")}: {date(key.created_at)} · {text("lastUsed")}: {key.last_used_at ? date(key.last_used_at) : text("neverUsed")}</p></li>)}</ul>{required && keys.data.length === 1 && <p className="text-sm">{text("lastKey")}</p>}<ProfileButton onClick={() => openPanel("add")}>{text("addKey")}</ProfileButton></>}
        {panel === "add" && <form onSubmit={addKey} className="space-y-4" noValidate><p className="text-sm">{text("addKeyHelp")}</p><UiInput ref={firstInput} label={text("keyName")} value={keyName} maxLength={128} onChange={(event) => setKeyName(event.target.value)} hint={text("keyNameHelp")} error={invalid && !keyName.trim() ? text("keyNameRequired") : undefined} /><p className="text-[13px] text-[var(--ui-text-muted)]">{text("globalImpact")}</p><div className="flex justify-end gap-2"><ProfileButton variant="secondary" disabled={busy} onClick={guard.requestClose}>{text("cancel")}</ProfileButton><ProfileButton type="submit" disabled={busy}>{text(busy ? "processing" : "addKey")}</ProfileButton></div></form>}
        {panel === "password" && <form onSubmit={savePassword} className="space-y-4" noValidate>
          ...
          <label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={showPasswords} onChange={(event) => setShowPasswords(event.target.checked)} />{text("showPasswords")}</label><p className="text-[13px] text-[var(--ui-text-muted)]">{text("globalImpact")}</p><div className="flex justify-end gap-2"><ProfileButton variant="secondary" disabled={busy} onClick={guard.requestClose}>{text("cancel")}</ProfileButton><ProfileButton type="submit" disabled={busy}>{text(busy ? "saving" : "save")}</ProfileButton></div>
        </form>}
        {panel === "identities" && <><ul>{identities.data.map((identity) => <li key={identity.id} className="border-b border-[var(--ui-border-soft)] py-3 first:pt-0"><div className="flex flex-wrap items-center justify-between gap-3"><strong className="min-w-0 break-words text-sm">{identity.email ?? identity.provider_id}</strong>{generalSettings.allow_user_external_identity_unlink && <ProfileButton variant="secondary" onClick={() => confirm({ kind: "identity", value: identity })}>{text("unlink")}</ProfileButton>}</div><p className="text-[13px] text-[var(--ui-text-muted)]">{identity.provider_type}: {identity.provider_id} · {text("created")}: {date(identity.created_at)}</p></li>)}</ul>{!generalSettings.allow_user_external_identity_unlink && <p className="text-sm">{text("managedIdentities")}</p>}</>}
        {error && !confirmation && <UiInlineMessage tone="error" role="alert">{error}</UiInlineMessage>}
      </div>
    </ProfileDialog>}
    {details && <ProfileDialog title={text("sessionDetails")} onClose={() => setDetails(null)}><dl className="space-y-3 text-sm">{[[text("name"), sessionDevice(details, text)], [text("ip"), details.ip_address ?? text("unknown")], [text("method"), details.auth_type === "password" ? text("password") : details.auth_type === "webauthn" ? text("passkeys") : details.auth_type === "recovery_code" ? text("codes") : details.auth_type], [text("created"), date(details.created_at)], [text("lastUsed"), date(details.last_activity_at)], [text("idleExpiry"), date(details.idle_expires_at)], [text("expiry"), date(details.absolute_expires_at)]].map(([label, value]) => <div key={label} className="grid gap-1 sm:grid-cols-2"><dt className="text-[var(--ui-text-muted)]">{label}</dt><dd className="break-words">{value}</dd></div>)}</dl></ProfileDialog>}
