...
def _portal_log_bucket_name(account_id: int, rgw_account_id: str | None, account_name: str) -> str:
    ...
    return f"bkr-portal-access-logs-{account_id}-{digest}"
...
def test_portal_storage_space_configures_server_access_logging_on_lab(
    ...
    try:
        ...
        policy_payload = _wait_for_value(
            ...
            lambda current: bool(
                _statement_by_sid(current.get("policy") or {}, "BucketReefPortalServerAccessLogging")
            ),
        )
        managed_statement = _statement_by_sid(policy_payload["policy"], "BucketReefPortalServerAccessLogging")
        ...
        manager_deny = _statement_by_sid(policy_payload["policy"], "BucketReefPortalManagerDeny")
