...
from app.db import S3Connection, User, UserRole, WebhookEndpoint
...
def _persist_targets(db_session):
    ...
    db_session.add(
        WebhookEndpoint(
            ...
            url="https://hooks.uncovered.example.test/callback?signature=must-not-appear",
            enabled=False,
        )
    )
    db_session.commit()
...
def test_preflight_lists_only_uncovered_hostnames(db_session):
    ...
    assert "webhook: hooks.uncovered.example.test" in rendered
...
def test_preflight_accepts_exact_and_explicit_wildcard_coverage(db_session):
    ...
    settings = Settings(
        ...
        webhook_allowed_hosts=["hooks.uncovered.example.test"],
    )
