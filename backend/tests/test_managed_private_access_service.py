...
def test_iam_provisioning_never_serializes_or_audits_secret_and_is_idempotent(db_session, monkeypatch):
    ...
    assert fake.created_users == [f"bkr-private-u{user.id}-acc{account.id}"]
    assert fake.created_keys == [f"bkr-private-u{user.id}-acc{account.id}"]
...
def test_iam_provisioning_accepts_builtin_full_access_when_catalog_omits_it(db_session, monkeypatch):
    ...
    assert result.status == "active"
    assert fake.attached == [(f"bkr-private-u{user.id}-acc{account.id}", policy_arn)]
...
def test_iam_provisioning_never_reuses_portal_identity(db_session, monkeypatch):
    ...
    assert provisioning.iam_username == f"bkr-private-u{user.id}-acc{account.id}"
...
def test_managed_iam_connection_delete_cleans_remote_resources_before_local_rows(db_session, monkeypatch):
    ...
    assert fake.deleted_keys == [(f"bkr-private-u{user.id}-acc{account.id}", "AK-MANAGED")]
    assert fake.deleted_users == [f"bkr-private-u{user.id}-acc{account.id}"]
