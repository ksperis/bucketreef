...
def classify(*, source: str, ref: str, protected: bool, mode: str = "auto", tag: str = "") -> str:
    ...
    if source == "push" and mode == "auto":
        return "integration"
    if source == "web" and ref == "main" and mode in {"qualify", "docs", "recover-release"}:
        return mode
...
def select(profile: str, paths: list[str] | None, *, ref: str = "main", version: bool = False) -> dict:
    if profile not in {"pr", "integration", "qualify", "release", "docs", "regression", "security", "secrets-history", "recover-release"}:
    ...
    elif profile in {"release", "recover-release"}:
