...
| `/manager/browser` embedded Browser | The active Manager `ctx`: Account root credentials, the selected RGW user's credentials, or an owned private connection's credentials. | `browser_enabled`, `browser_manager_enabled`, `manager_enabled`, plus context-specific authorization. Account links require `account_administrator` and `allow_manager_browser_data_access = true` on the same row; RGW-user links aggregate the flag across direct and groups; private connections require `access_manager = true` and `access_browser = true`. | Disabled by default. Existing association rows are not enabled by migration. Shared connections are always rejected, and every request must carry `X-S3-Workspace: manager-browser`. | The Manager topbar and `ctx` remain authoritative. The UI displays the effective S3 identity and warns when RGW logs attribute operations to an admin/root or shared RGW identity instead of the UI user. |
| `/portal/storage-spaces/:spaceId` locked Browser | Portal execution identity resolved for the selected account and DB-backed Storage Space. | `browser_enabled`, `browser_portal_enabled`, `portal_enabled`, Portal account role, `X-S3-Workspace: portal`, active Storage Space visibility, DB grant role, and the explicit Portal profile. | Enabled by default but locked to one active Storage Space. Bucket switching and technical S3 tools are hidden. The Portal passes already-resolved capability facts for upload, folder creation, deletion, restore, and sharing; Browser never reconstructs authorization from raw roles. Viewer capabilities remain read-only. Object details and deleted-object history open in the shared drawer without leaving the Storage Space page. | This is the end-user file profile. Archived Storage Spaces remain blocked even if older credentials still have storage-side access. Backend Portal checks use DB metadata and grants; IAM policies are projection/enforcement for S3 keys, not the source for listings or roles. |
...
| Technical Browser tools | Technical actions, advanced columns and search, versions, metadata mutation, batch operations, and bucket shortcuts can make a simple file workflow look like an operator console. | Root checks effective `browser_advanced_features_enabled`, presented to administrators as **Technical S3 tools**. This functional profile does not control display preferences. Every root `/browser` user can choose Compact or Comfortable and independently show Folders. Root UI state v3 stores one set of root-only display preferences. Embedded surfaces receive their profile and density explicitly and never use root storage. | Technical tools are unavailable to Standard and Portal profiles. Root Browser defaults to Compact with Folders hidden; Manager and Ceph Admin embeds are compact, while the locked Portal embed is comfortable. Compact combines the path and icon actions on one row when width permits; Comfortable shows labeled actions on that row only at extra-wide viewports and otherwise reserves a stable second row so selection does not shift the list. A row click opens its primary destination; checkboxes alone control selection. The action-bar More menu contains only secondary selection actions; path and Browser settings return when the selection is cleared. The unselected More menu avoids duplicating Upload and New folder, and separates current-path actions from technical tools. Object, path, and bucket details use one contextual drawer and never depend on selection. Advanced path details describe the virtual S3 prefix without loading misleading folder metadata; their recursive content count runs only on explicit request. The dedicated Browser exposes a read-only Bucket details summary; Manager and Ceph Admin embeds reuse the capability-filtered bucket editors. | Add more granular admin toggles only if the technical bundle remains too broad. |
...
| Preview, properties, raw metadata, tags, and advanced object details | Metadata, tags, storage class, headers, ACL-like details, legal hold, retention, and restore controls are operational details that may be inappropriate in end-user spaces. | Primary activation always opens the common drawer on Preview. Standard adds a read-only Details tab. Advanced adds Versions, editable Properties, Access & Protection, and Archive when applicable. Portal composes Preview, History, Sharing, and Details through one shared Storage Space drawer in both the Portal page and a root Browser Portal context; only the Portal page persists drawer state in its URL. | Technical detail tools are disabled in Standard and Portal. Portal sharing and restore actions remain filtered by resolved capabilities. | Keep the common preview threshold and supported-type policy synchronized across surfaces. |
...
Manager Browser object operations follow the same audit boundary and are not
persisted in application `audit_logs`. The `manager-browser` header is an
authorization surface, not a catalogue or stored selection. Authorization is
recomputed on every request so association or connection revocation takes
effect immediately.
...
- Do not add a `/portal/browser` route. Portal may embed the main Browser on
  ...
  Portal functional profile, Standard layout, comfortable density, resolved
