# Upgrade to BucketReef 0.2.0

BucketReef 0.2.0 is an intentionally incompatible product rename. It does not read
...
Any 0.2.0 container published before this replacement is withdrawn. Do not use
those artifacts as an installation or migration source; redeploy from the
verified BucketReef digests published by the replacement release.

## Contract changes

| Before 0.2.0 | BucketReef 0.2.0 |
|---|---|
| Project and repository `s3-manager` | `bucketreef` |
| Frontend package `s3-manager-frontend` | `bucketreef-frontend` |
| Compose project inferred as `s3-manager` | Explicit project `bucketreef` |
| `S3_MANAGER_TAG` | `BUCKETREEF_TAG` |
| `S3_MANAGER_BACKEND_IMAGE` | `BUCKETREEF_BACKEND_IMAGE` |
| `S3_MANAGER_FRONTEND_IMAGE` | `BUCKETREEF_FRONTEND_IMAGE` |
| `S3_MANAGER_DB_BACKUP_VERIFIED` | `BUCKETREEF_DB_BACKUP_VERIFIED` |
| JWT issuer `s3-manager` | `bucketreef` |
| JWT audiences `s3-manager-ui`, `s3-manager-api`, `s3-manager-pre-auth` | `bucketreef-ui`, `bucketreef-api`, `bucketreef-pre-auth` |
| Helm chart and release examples `s3-manager` | `bucketreef` |
| PostgreSQL user and database `s3manager` | `bucketreef` |
| Generated Ceph, IAM, and S3 prefix `s3m-` | `bkr-` |
| Policy SID prefix `S3Manager` | `BucketReef` |
| GHCR packages `s3-manager-backend` and `s3-manager-frontend` | `bucketreef-backend` and `bucketreef-frontend` |
...
5. Export an inventory of administrative Ceph identities, capabilities, and
   the BucketReef connection that consumes each credential.
...
Do not set `BUCKETREEF_DB_BACKUP_VERIFIED=true` until the restore test has passed.
...
removed remotely. After BucketReef is running, recreate the recorded accesses. The
new principals use the `bkr-private-*` contract. Validate each connection before
discarding the inventory.

### Administrative Ceph identities

Create the replacement `bkr-admin`, `bkr-supervision`, and `bkr-ceph-admin`
principals before revoking `s3m-admin`, `s3m-supervision`, and `s3m-ceph-admin`.
Copy the required capabilities exactly, update the stored credentials in BucketReef,
...
from the former `s3m-portal-access-logs-*` target, then enable logging in BucketReef
to create a `bkr-portal-access-logs-*` bucket. Copy any history that must remain
available, verify that BucketReef can read it and that new events arrive, then remove
...
3. Create `bucketreef_backend-data` and copy the stopped SQLite data into it. For
   example, after confirming both exact volume names:

   ```bash
   docker volume create bucketreef_backend-data
   docker run --rm \
     -v s3-manager_backend-data:/from:ro \
     -v bucketreef_backend-data:/to \
     alpine:3.20 sh -c 'cp -a /from/. /to/'
   ```

4. Rename all `S3_MANAGER_*` variables according to the contract table and use
   the BucketReef compose file. Preserve the JWT and credential keyrings.
5. Start the `bucketreef` project, apply migrations once with the verified-backup
...
database with the provider tooling, create or grant the `bucketreef` database role
...
3. For an external database, point the new release at the restored or retained
   database. For chart-managed persistence, restore into a new BucketReef PVC or
...
- Verify access to every encrypted storage connection using the retained
  credential keyring.
- Exercise each recreated `bkr-*` private access and administrative principal.
- Confirm there are no active migration policies with old or temporary SIDs.
- Confirm Portal logs are written to and readable from `bkr-*` targets.
- Verify that deployed images, Helm resources, Compose project metadata,
  browser titles, and API metadata use BucketReef identifiers only.
